...
const getDataComment= async (req)=>{
    const perPage=10;
    ...
    const [r1]=await db.query("SELECT COUNT(1) num FROM `comment` ORDER BY comment.id DESC");
    ...
    const sql =`SELECT * FROM comment ORDER BY comment.id DESC LIMIT ${(page-1)*perPage}, ${perPage}`;
...
router.get("/:page?", async (req, res) => {    
    const output = await getDataComment(req); 
    res.json(output);
    // console.log(output)
...
router.put('/:id?', upload.none(), (req, res)=>{
   ...
   console.log(id)
    ...
    // }
   const sql = "UPDATE `comment` SET ? WHERE cid = ? ";
...
router.delete('/:id', (req, res)=>{
    ...
    const sql = "DELETE FROM `comment` WHERE `comment`.`id` = ? ";
...
// db.query(`SELECT * FROM comment`).remove(req.params.id, function(err, obj) {
// if (err) {
